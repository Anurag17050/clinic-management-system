...
# ── GET all departments (with head doctor + staff count) ──
...
def get_departments():
    ...
    cur.execute("""
        ...
        GROUP BY d.dept_id, d.dept_name, d.location,
                 d.head_doctor_id, doc.first_name, doc.last_name,
                 doc.specialization
        ORDER BY d.dept_id
    """)
    rows = cur.fetchall()
...
def delete_department(dept_id):
    ...
    try:
        cur.execute(
            "SELECT COUNT(*) FROM DOCTOR WHERE dept_id = :1", [dept_id]
        )
        doc_count = cur.fetchone()[0]
        if doc_count > 0:
            return jsonify({
                "error": f"Cannot delete — {doc_count} doctor(s) still assigned to this department."
            }), 400

        cur.execute(
            "SELECT COUNT(*) FROM STAFF WHERE dept_id = :1", [dept_id]
        )
        staff_count = cur.fetchone()[0]
        if staff_count > 0:
            return jsonify({
                "error": f"Cannot delete — {staff_count} staff member(s) still assigned to this department."
            }), 400
